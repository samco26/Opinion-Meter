...
- **"Analyse this page's subject."** Under the site's reading the badge carries a button (it stays in place, between the figures and the summary, while the site's own card is grown). Pressed (after a one-time note saying what is sent), the page's visible text — reviews and comments first, up to about twelve thousand words, never anything typed into forms — goes to the server's new page door once, and a ring of green-into-red light circles the button while the page is read. The server names the page's subject, then at the same time sweeps the platforms for it and reads the page for the opinions written on it — each copied verbatim and checked against the page, so no quote is invented — then writes one reading. The same box grows into the page card: the subject's bar and figures, a summary, the sources (the page's own favicon among the platform tiles, each opening its quotes), and the pros and the cons in two columns of green and red bubbles, up to six each, most repeated first, each opening the words behind it. A review found on the page counts as three platform posts. Fewer than eight opinions in all: "Subject does not possess sufficient sentiment footprint" under the subject's name; a page about nothing in particular: "This page seems to be about nothing in particular". The page card scrolls inside, stays open while the page is scrolled, and once made reopens at once (the server holds it a quarter of an hour; the page's text is never kept).
