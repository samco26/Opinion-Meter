...
- The health door reports the server's real version (it had "1.0.0" written into it; it now reads the package).
...
- **"Analyse this page's subject."** The badge is two sheets: the site's pill on top and, tucked behind it, a tray whose lower strip is the button, centred, with a faint green-into-red tint sweeping slowly across it. Both take the extension's own type and darken under a pointer that turns to a hand. The pill opens into the site's card on a press (no longer on hover); the strip rides down with it. Pressing the strip — after a one-time note saying what is sent — sends the page's visible text (reviews and comments first, up to about twelve thousand words, never anything typed into forms) to the server's new page door once, and a ring of green-into-red light circles the sheet while the page is read: the pill and strip together at rest, the tray alone under an open site card. The server names the page's subject, then at the same time sweeps the platforms for it and reads the page for the opinions written on it (each copied verbatim and checked against the page, so no quote is invented), then writes one reading. The tray then grows into the page card beneath: the subject's name, bar and verdict as its header (the pill above stretches to the same width), the figures, a summary, the sources (the page's own favicon among the platform tiles, each opening its quotes), and the pros and the cons in two columns of green and red bubbles — short phrases ("Excellent noise cancelling", "Some compatibility issues"), up to six each, most repeated first, each opening the words behind it. Only the pros and cons scroll. The two cards open and close on their own: the site's by its ×, a press on it or scrolling the page; the page's by its header, its × or a press anywhere on it but the lists; Escape closes the topmost, a press outside closes both. An open card tints grey under the pointer wherever a press would fold it. A review found on the page counts as three platform posts. Fewer than eight opinions in all: "Subject does not possess sufficient sentiment footprint" under the subject's name; a page about nothing in particular: "This page seems to be about nothing in particular". Once made, a page's reading reopens at once (the server holds it a quarter of an hour; the page's text is never kept).
...
- **Motion, made one.** Every box that grows or shrinks — the site's card, the tray, the pill stretching to the tray's width, the figures row, the × — moves in 340 ms on one curve, and every surface, tint and fade takes 160 ms; the tray and the ring, which live beside the card, now share that curve (they had been jumping). A page card is laid out at its final width while its tray widens, as the site card is; it fades as the tray folds; an open tray rides the site card's growth beneath it, its lists untouched.
