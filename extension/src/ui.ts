/* What the hands draw: the bar on a result's site-name line (and the
   card for the query), the tooltip, and the drawer that opens over the
   page. Everything lives in its own shadow root so Google's styles and
   ours never touch, in the palette and glass of the card it opens. A
   result's bar stays invisible until a reading exists; a subject with too
   few opinions gets an empty outline. The tooltip hangs off the page root
   so Google's transformed containers cannot move it. */

import type { Gauge } from "./shared";

const BAR_CSS = `
:host{all:initial;display:inline-block;vertical-align:middle;margin-left:8px;--om-size:12px}
:host([data-opinion-meter="query"]){display:block;margin:0}
:host([hidden]){display:none!important}
.bar{display:inline-flex;align-items:center;gap:7px;margin:0;padding:0;border:0;background:transparent;font:12px/1.2 Helvetica,"Helvetica Neue",Arial,sans-serif;color:#153f43;cursor:pointer;position:relative;white-space:nowrap;vertical-align:middle;text-align:left;opacity:0;transition:opacity 280ms ease}
.bar.shown{opacity:1}
...
/* "38% negative", with the number of opinions only where asked for. */
function countText(gauge: Gauge, withCount: boolean): HTMLElement {
  ...
  if (withCount) count.append(` · ${gauge.count} opinions`);
  return count;
}
...
export function createBar(opts: { big?: boolean; title?: string; dark?: boolean; size?: number; onOpen: (gauge: Gauge | undefined, anchor: DOMRect) => void }): Bar {
  ...
  const render = (state: BarState) => {
    ...
    bar.append(segments(gauge), countText(gauge, false));
    if (gauge.simulated) bar.append(el("span", "tag", "estimated"));
    detail = `${gauge.count} opinions. ${gauge.simulated ? "Unverified word-count estimate. " : ""}${gauge.sentence}`;
