...
export function createBar(opts: {
  ...
  const showDetail = (view: typeof pageView) => {
    ...
    if (view.kind === "point") {
      ...
      pdetail.append(el("p", "quiet", `This card is about ${page.subject}, the subject of the page you are on. The reviews and comments found on the page itself lead the reading: each counts as three platform posts. Public posts about the subject from the platforms add to it.${page.rating ? ` The page's own rating, ${page.rating.value} out of ${page.rating.best}${page.rating.count ? ` from ${compact(page.rating.count)} ratings` : ""}, joins the meter as a block of up to 1,000 votes, read as the share who liked it on that scale.` : ""}`));
