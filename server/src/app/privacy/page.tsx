  ...
  return (
    <main className="page">
      <section className="glass page-card prose">
        ...
        <h2>On other websites: &ldquo;Take the bar with you&rdquo;</h2>
        <p>The extension also runs on other websites, to show at the top right of a site the reading you saw for it on Google. There it reads only the page&apos;s address, inside your browser, to find a reading it already holds. It sends nothing about those pages to the server or anywhere else, and keeps the readings it carries in the browser only until you close it. This is on unless you turn it off in the extension&apos;s menu, behind its toolbar icon.</p>
        <h2>Why the browser says “read and change data”</h2>
        <p>Browsers grant page access as a combined capability: the same permission that lets an extension draw a bar also allows it to read and change that page. They do not offer a draw-only permission. Opinion Meter does not read passwords or form entries; it adds its own bars and drawer. Removing automatic page access would require you to activate it separately on pages and would stop the card following automatically. Turning off “Take the bar with you” stops drawing site cards but does not revoke the browser permission; you can restrict website access in your browser&apos;s extension settings.</p>
