...
export async function collectAdaptive(
  subject: string,
  sources: SourceId[],
  options: { link?: string; domain?: string; to?: Date; collect?: typeof collectAll; depth?: "lite" | "full"; budgetMs?: number } = {},
): Promise<{ items: SourceItem[]; statuses: SourceStatus[]; window: SearchWindow }> {
  const { link, domain, to = new Date(), collect = collectAll } = options;
  ...
  for (const windowMonths of SEARCH_MONTHS) {
    ...
    months = windowMonths;
    const batch = await collect(active, { subject, link, domain, from: monthsBefore(to, months), to, depth: options.depth, memo, previousItems: [...items.values()], timeoutMs: Math.min(9000, Math.max(1, budget - (Date.now() - started))) });
