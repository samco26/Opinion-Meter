...
/* Algolia exposes at most its configured pagination window (normally
   1,000 hits). Use the largest page and all indexed dates in the drawer.
   A separate comment query finds mentions buried in unrelated threads. */
async function collectFull(opts: CollectOptions): Promise<Collected> {
  ...
  try {
    ...
    if (comments < 1000) {
      const mentions = await request<Comment>({ query: opts.link ?? opts.subject, tags: "comment", restrictSearchableAttributes: "comment_text" });
  ...
  return { items, canExpand: false, status: { source: "hn", availability: !items.length ? "unavailable" : limited || failure ? "partial" : "ok", itemsAnalysed: items.length, note: `Collected ${items.length} Hacker News entries across all indexed dates. Search requests use up to 1,000 hits; up to 1,000 comments per reading. ${failure || (limited ? "The API or reading budget limited coverage." : "Reached the end of the returned matches.")}` } };
}
