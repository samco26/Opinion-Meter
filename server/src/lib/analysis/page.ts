...
- pros: up to ${POINTS_MAX} distinct recurring points in the subject's favour that people actually make, each a short phrase of at most 50 characters naming the point itself, in sentence case with no full stop — "Excellent noise cancelling", "Long battery life", "Good value when discounted" — never who says it or how many: not "buyers report", "owners say", "some find", "many users", "reviewers praise"; a qualifier belongs to the point ("Some compatibility issues", "Occasional connection drops"). Give each point the numeric references that actually support it, page entries first, and require at least two independent entries per point. cons: likewise, the recurring complaints, shortcomings and reasons not to buy, go, watch or use — "Fragile hinges", "Bulky non-folding case", "High price". Combine paraphrases, do not force equal numbers, return fewer or none when the evidence is thin, and sort by recurrence. Never invent references or quotations.
