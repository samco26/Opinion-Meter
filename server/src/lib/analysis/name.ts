/* The batched naming call: the query and every result no rule could
   ...
   subject names, or as nothing. People are never subjects. Answers are
...
const Named = z.object({
  query: z.object({ kind: Kind, name: z.string() }),
...
- A named individual, living or dead, is person. Never return a person's name under any other kind.
...
const queryKey = (query: string) => `name:q:${hashKey(query.toLowerCase().trim())}`;

function toSubject(kind: string, name: string, url?: string): Subject | null {
  const clean = name.replace(/\s+/g, " ").trim();
  if (!NAMEABLE.has(kind) || !clean) return null;
  if (kind === "article") {
...
export async function nameSubjects(query: string, unknown: Array<RawResult & { i: number }>, timeoutMs: number): Promise<Naming> {
  ...
  try {
    ...
    if (!storedQuery) {
      querySubject = toSubject(out.query.kind, out.query.name);
