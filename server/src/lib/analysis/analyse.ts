...
- ${CLASSIFY_RULES}
- Extract up to 20 distinct recurring opinions across the relevant entries, ideally 5 to 20 only when supported. Each is one concise sentence (preferably under 100 characters), its positive/neutral/negative sentiment, and the refs that actually support it. Require at least two independent relevant entries per sentence. Combine paraphrases, do not force equal positive and negative counts, and return fewer or none when evidence is thin. Sort by recurrence. Never invent references or quotations.`;
