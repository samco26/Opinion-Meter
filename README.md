...
- **Firefox's declaration (v0.16.10):** the Firefox label names the same two things in Mozilla's categories — "search terms" and "website content" — under `browser_specific_settings.gecko.data_collection_permissions`, which Firefox Add-ons requires of every new add-on and Firefox 140+ shows at install. Anything more sent to the server must be added there in the same commit as here.
