...
- **Permissions requested at install:** the Google search domains only, plus local storage for the install token and the config, and "scripting" (used only for the point below). No "all sites" permission at install.
- **"Take the bar with you" (v0.11):** an optional permission for every site, asked for only when the reader turns it on in the settings (a one-time note on Google points there). With it on, a site script runs on every page but Google's search pages, reads only the page's address inside the browser, and asks the brain for a reading it already holds from Google; nothing about those pages is sent to the server or anywhere else. Carried readings live in session storage (gone when the browser closes), a × hides the card on a site until its bar is next loaded on Google, and the settings turn it off (permission removed, script unregistered).
