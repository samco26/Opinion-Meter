...
### Current behaviour — 23 September 2026, v1.0.4

v1.0.0 (the day's 0.17.0 work, declared version one) makes the bar and the card one reading, puts the card on every site, and adds "Analyse this page's subject". Beside every Google result the verdict names its subject ("63% positive opinion of Sony Australia"); the badge's cards fold on a press, with no ×s. **One reading:** the bar's quick reading is the reading. The server keeps the bar's sample (the entries and each one's verdict) for a quarter of an hour, and a card asked for in that time — the recurring opinions list, the prefetched card — is built from that very sample with the verdicts locked, so its numbers are the bar's to the digit; the list no longer pushes numbers back into the bar, on the page or in memory. A card asked for later (the sample aged out, or no bar came first) is read afresh and becomes the subject's reading for the next search; a bar already on a page keeps what it shows. X is read for the bar and the card alike, once per subject. **The card on every site:** the badge at the top right now appears on every site, not only those reached from Google: the site's reading is fetched by the site's address (the domain, never the page's path) and the name it declares for itself, and shared with the site's other tabs; the × hides it on that page until the page is next loaded; the menu switch ("Show the card on every site") turns it off everywhere. **The second sheet:** the badge is two sheets — the site's pill on top and, tucked behind it, a tray whose lower strip is the button "Analyse this page's subject" (centred, a faint green-into-red tint sweeping across it). The pill opens into the site's card on a press; the strip rides down with it. Pressed (after a one-time note), the page's visible text — reviews and comments first, up to about twelve thousand words, never anything typed into forms — goes to the server once; a ring of light circles the sheet while the page is read. The server names the page's subject, then at the same time sweeps the platforms for it and reads the page for the opinions written on it (each copied verbatim and checked against the page), then writes one reading: a bar and figures, a summary, the sources (the page's own favicon among the platform tiles), and the pros and the cons in two columns of green and red bubbles, each opening the words behind it. A review found on the page counts as three platform posts. Fewer than eight opinions in all: "Subject does not possess sufficient sentiment footprint"; a page about nothing in particular: "Cannot determine single subject", centred on the strip (two even lines, the strip growing, when the pill is too narrow for one); a page the server could not read at all is not guessed at — the strip says so and can be pressed again. The tray then grows into the page card beneath the pill (which stretches to the same width); only its pros and cons scroll; the two cards open and close on their own (the site's by its ×, a press on it or scrolling; the page's by its header, its × or a press anywhere on it but the lists; Escape closes the topmost, a press outside both), and an open card tints grey under the pointer wherever a press would fold it. Sending page text and site addresses is a change to what leaves the browser: section 9, the privacy page, the menu and the Firefox declaration say so.

#### Earlier — 16 September 2026, v0.16.6
...
Summaries and the bar's sentence describe the subject's **general standing**, never the news of the moment; a separate **"Lately"** line appears only when a development of the past two weeks has clearly swung opinion, resting on at least three dated entries, with roughly when it happened. Momentary "is it down?" reports do not count as opinions. The sample the model reads takes turns across platforms and caps any single thread, so one busy platform or one outage thread cannot drown the rest; search windows widen platform by platform.

Result bars are drawn on a layer over the page and pinned right after the site name's text (or after the dots where the name shares its line, under a Shopping tile's seller line, after a source label in an AI answer's panel, or in the top-right corner of a card that names its site at the bottom), with "42% positive" and no opinion count, fade in, adapt to Google's dark theme, and are invisible until a reading exists — an empty outline when the evidence is thin. The query's card mirrors Google's pill buttons on the results pages and sits beside a knowledge panel's title. The drawer is anchored to the page beside its bar, shows the three percentages under the bar, a "Lately" line when there is one, and only its recurring opinions scroll. Four packages come from one build: Chrome (which also installs on Brave, Opera, Vivaldi and Arc), Edge (the same package under its own name, for the Edge store), Firefox (its own manifest), and Safari (the folder in Safari's shape, which a Mac job in CI turns into an unsigned Mac app on request; signing and the App Store need an Apple developer account). On sites opened from Google, "take the bar with you" shows the reading as a frosted card at the top right that can be dragged anywhere and fades while scrolled.
...
**General versus recent.** The summary and the bar's sentence describe how people regard the subject over time and never hinge on an incident the reader has no context for. When a development of the past two weeks — an outage, a redesign, a controversy, a price change — has clearly swung how people feel, with at least three dated entries behind it, one "Lately" sentence names it with roughly when it happened; otherwise nothing, and when in doubt nothing. A bare report that something is down right now is not an opinion of it.
...
- **Permissions requested at install:** the Google search domains, every site (for the card on every site, below; the browser says so at install), and local storage for the install token, the config and the one-time note's flag.
- **The card on every site (v0.11 as "take the bar with you", on by default from v0.12, on every site from v0.17.0):** a site script runs on every page but Google's search pages and draws the badge. It asks the brain for the site's reading: the one carried from Google when the reader came from there (nothing is sent), else one fetched by the site's address — the origin (scheme and domain), never the page's path or query — and the name the site declares for itself (`og:site_name`), which the brain then files for the site's other tabs. Carried and fetched readings live in session storage (gone when the browser closes). The × hides the badge on that page until it is next loaded; the menu switch ("Show the card on every site") turns it off everywhere, and the script then draws and sends nothing.
- **"Analyse this page's subject" (v0.17.0):** only when the reader presses the badge's second button, and after a one-time note, the page's visible text (reviews and comments first, up to about twelve thousand words), its title, its address and any JSON-LD it carries go to the page door once. Menus, headers, footers, scripts and everything typed into forms are skipped. The server reads the text and forgets it; only the finished reading is held, for a quarter of an hour.
- **What is sent to the server, in all:** the titles and addresses of results on Google search pages and the query; every visited site's origin and declared name (unless the card is off); a page's text on the button's press. No cookies, no account details, no page paths.
- **Firefox's declaration (v0.17.0):** the Firefox label names these in Mozilla's categories — "search terms", "browsing activity" (the site addresses) and "website content" (result titles and, on the press, page text) — under `browser_specific_settings.gecko.data_collection_permissions`, which Firefox Add-ons requires of every new add-on and Firefox 140+ shows at install. Anything more sent to the server must be added there in the same commit as here.
- **What the server keeps:** derived answers for 24 hours; a finished card, quotes included, and the bar's sample (the entries it was counted from) for a quarter of an hour, so the list and the card agree with the bar; never a page's text; request logs without raw queries beyond what abuse prevention needs, for a short fixed period. The privacy page states the period.
- **What the server sends to the AI:** comment text and titles for classification, and on the press the page's text; never usernames.
...
| `/api/page` | POST `{ url, title, site?, description?, text, data? }` | One reading of the page the reader is on, from its subject: the page named by the model (a topic, a search or a listing is nothing in particular), then the platforms swept and the page read for its own reviews at the same time, then one pass over both; a page review weighs three platform posts. The page's own overall rating is read by a small call of its own, so a slow read of the page's opinions cannot lose it; that read is tried once more, shorter, when it does not land; and when neither lands the door answers 502 rather than publish a reading the page had no part in. Sent only on the badge button's press; the text is never kept; the finished reading is held 15 minutes. Needs the AI key. |
...
| 2026-09-15 | The bar and the drawer are one reading: the bar reads the same three years, platforms and 250-entry sample as the card (a cheap model classifies), the card's numbers replace the bar's whenever a card is made, and an opened drawer sends its numbers back to the bar. Sites outside the table are named by the AI from address and label, so two ABC News are told apart. X falls back to the last seven days when the token has no archive access. Stars are gold and shown for every subject; Shopping bars rate the seller; thin readings are hover-only; no card is prepared ahead on the Videos tabs. A CHANGELOG.md is kept per version (AGENTS.md rule). |
| 2026-09-15 | Stars are gone from the drawer (reversing the row above). Result bars return to the site-name line where that line is free (the All tab), else after the dots (Videos, Forums). The query card sits right-aligned on the knowledge panel's title lines, between the text and any logo, or goes into the flow when there is no room. The bar layer sits one step under Google's search header in the stacking order. An AI answer's source bars are measured from the label's visible end, one width for all; the narrow card above them keeps only the name. A loading query card shows only "What people think" and the moving bar. Built-but-unseen Google pop-ups (the ad centre) and anything in a box fixed to the window get no bar. The drawer's card never scrolls (only the recurring opinions list does) and the loading drawer is a short strip. |
| 2026-09-15 | Result bars sit right after the site name's text. The drawer is anchored to the page beside its bar (it scrolls with the results), fades out on close, and shows no cover while it loads. Shopping: every tile's seller line gets a bar, found from the tile's price. The query card on results pages mirrors Google's "Search … on Google" pill (46 px, 30 px corners, 14 px Google Sans). The brain remembers gauge answers and prepared cards for ten minutes, so tab changes within one search ask the server nothing. Knowledge panel: the card sits right after the dots with "What people think" above the bar and the verdict to its right; "Mixed" reads "Mixed opinion". An AI answer's source card that names its site at the bottom gets its bar in the top-right corner. "How it works" sits under the opinion count so the recurring opinions end the card; the loading phrase fades every 3.2 s, left-aligned. "Lately" reaches back 14 days (was 60) and needs three dated event entries (was two) about a development that clearly swung opinion. |
| 2026-09-15 | One look, taken from the query card's pill: Google Sans, ink-grey text, the pill's light grey for the hover card and the drawer (dark grey with light text on Google's dark theme, the drawer too, still slightly see-through). "How it works" returns to the bottom of the drawer (reversing the row above). "Recurring opinions" in sentence case; opinion cards without an outline; hover lift on the close button and platform icons; green-into-red loading sweep. The knowledge-panel card's bar sits on the subtitle's line with "What people think" in full ink above it. The sources panel is found by "Show all", "Show more" or "Show less", and bars inside it follow its own scrolling and are trimmed at its edges. |
| 2026-09-15 | "Take the bar with you" (v0.11): opt-in, asked once later (a note on Google after the first reading; the permission itself is granted in the settings, never at install). A site whose bar was loaded on Google shows the pill fixed at the top right of every page of that site, 224 px wide like Google's pill buttons, name + bar + verdict, with a × that hides it there until the bar is next loaded on Google; any page of that site counts, however it was reached. The reading is the brain's memory of the Google reading, kept until the browser closes; nothing about visited pages is sent to the server. The drawer opened from it stays fixed with it. Also fixed in v0.10.1: the drawer is dark on the dark theme (frame and embed keep one colour scheme), the loading sweep is two solid blocks, and expanded sources panels no longer keep bars on the hidden collapsed copy. |
| 2026-09-15 | "Take the bar with you" is on by default (v0.12), which means naming every site in the label so the browser says so at install; the one-time note and the optional permission are gone, and the menu behind the toolbar icon turns it off. The toolbar icon opens a menu (popup) styled like the drawer, not the settings page; the same page serves as the settings page in a tab (the embedded settings dialog resized itself endlessly). The site's card is solid at the top of the page, faint once the reader scrolls at all (still clickable, solid under the cursor), solid again at the very top. Drawers sit one step under Google's search header like the bars. On the dark theme every text takes the light ink (the drawer's text had inherited the page body's dark colour). |
...
| 2026-09-16 | One card that grows (v0.15): the drawer and the hover card are gone; a bar's own background stretches down on hover (the sentence, the three figures under the bar) and on click into the full card, which loads inside it. Same for the query pill, the small bars beside results (their name appears as they grow) and the site's card. It grows leftwards or upwards when the window's edge is near, never taller than the window; the query pill in the results flow is lifted onto the layer while open so the results never shift. The full card starts at the summary (no second bar). On a site, scrolling closes an open card; the pill's bar fills the room the name leaves and a long name widens the pill. |
| 2026-09-16 | The design handoff of 16 September is the look (v0.16), copied to the letter: hairline bars in the page's greys and font, colour only in bars and 5 px dots, the per-result bar under the site's name across the name-and-address column (a 5 px gap opened under the name), the query line "What people think of Kia" (the owner keeps that headline), fixed 34 px story bars, the source card at 460/360 px with dots, summary, tiles, chip, count, divider and footer, recurring opinions as quiet rows with a 2 px stance tick, the badge below a site's header. Loading keeps the green-then-red sweep; too few opinions is a plain grey track. The growing animation stays, and the header never moves while the card grows. 17 September: the per-result bar starts at the result's left edge (level with the title) and the verdict sits 6 px after the name on its exact line; the grown card grows from the bar's own box and shrinks back to it, contents laid out at their final width from the first frame, with a look-alike favicon in place. |
...
| 2026-09-18 | v0.16.10: the Firefox package declares the data it sends (search terms, website content) for Firefox's built-in consent prompt, as addons.mozilla.org requires of new add-ons, and needs Firefox 140 or later. Sent data unchanged. |
| 2026-09-21 | v0.17.0, one reading: the bar's quick reading is the reading; the server holds its sample (entries and verdicts) for a quarter of an hour and builds the card from it with the verdicts locked, so the list and the card carry the bar's numbers to the digit; the list no longer sends numbers back to the bar, and the server no longer overwrites a bar's reading with a card built from its own sample (a card read afresh — the sample aged out, or no bar came first — still becomes the subject's reading for the next search). X is read for the bar and the card alike (the owner: "include X in both"; up to US$0.10 of posts per new subject, on the bar now). Fixes the rating changing when "See recurring opinions" loaded. |
| 2026-09-21 | v0.17.0, the card on every site and "Analyse this page's subject" (the owner's answers of 21 September): the badge appears on every site, its reading fetched by the site's origin and declared name when none was carried from Google (the owner chose this over an on-tap reading, knowing every visited site's address then reaches the server); the × hides it on the page until reload; the menu switch hides it everywhere; the fade on scroll stays. The second row is a button, "Analyse this page's subject" (the owner's wording); after a one-time note the page's text is sent once; a green-into-red ring circles the whole badge while the server reads (21 Sep, later: the badge is two halves cut by a line, both darkening under the pointer; only the pros and cons scroll; pros and cons are short phrases); the page card is the same 360 px box, scrolls inside, stays open while the page is scrolled, and shows the subject's bar and figures, summary, sources with the page's own tile, and pros and cons as green and red bubbles (up to six each, most repeated first, each opening the quotes behind it; an empty column reads "Nothing recurring"). A page review counts as three platform posts (the owner chose "page weighs more" over "page decides" and "page leads the words only"). Too few opinions in all: "Subject does not possess sufficient sentiment footprint" under the subject's name; a page about nothing: "This page seems to be about nothing in particular". Privacy section, privacy page, menu, store notes and the Firefox declaration (now searchTerms, browsingActivity, websiteContent) updated in the same commit. |
| 2026-09-21 | v0.17.0, the badge as two sheets (the owner's sketch and answers of 21 September, later): the site's pill on top, a tray tucked behind it whose lower strip is the "Analyse this page's subject" button (centred; a faint slow green-into-red sweep; darkening whole, corners included, under the pointer, pulling out a touch); the pill opens on a press only (hover-to-expand dropped); the strip rides down with the open site card and grows into the page card beneath, the pill stretching to the tray's width; each card opens and closes on its own (stack, as the owner asked, "allow user to expand or close each independently"); a press anywhere on an open card but its lists folds it, and an open card tints grey under the pointer; the ring circles the pill and strip at rest, the tray alone under an open card. All motion 340 ms on one curve, all surfaces and fades 160 ms. Built and checked on a side branch with a stubbed test page before merging. |
| 2026-09-21 | v1.0.1, from the owner's screenshot of the badge on a Wikipedia page: the site card's summary no longer ends in a cut ("…becomes less trust…" — the server cut a card's first sentence at 160 characters; the card wraps, so the whole sentence goes), and the open badge's bar shows its grey neutral part (the head was never drawn again as the card opened, so it kept the two-part bar it has at rest). A debug audit of the badge on a stubbed page and of the four packages followed; the packages' scripts are byte-identical, the manifests differ only where each browser requires. |
| 2026-09-21 | v1.0.2 (the owner, evening): in a result's expanded card on Google the favicon, the site's name, the bar and the "xx% positive opinion of xx" label are links to that result's own address (the specific page Google lists, never the site's front door); the ring while a page is read circles the whole badge — pill or open card and tray together — not the tray alone under an open card. |
| 2026-09-21 | v1.0.3 (the owner, from a screenshot of the badge on Amazon): a page read as nothing in particular says "Cannot determine single subject", centred on the strip, in place of "This page seems to be about nothing in particular"; the strip grows to a second line when the pill is too narrow for one, for this and for the "insufficient footprint" line; the server drops its held "nothing" answers so the old words are not served. |
| 2026-09-23 | v1.0.4, from the owner's screenshot of a 4.3-star Amazon listing read as 81% negative: a page reading that loses the page itself is not published. The page's own star rating is read by its own quick call (a shop publishes no machine-readable rating, so this is the only way it is found); the long read of the page's opinions is tried a second time, shorter, when the first does not land; and when nothing comes back from the page at all the server says so — the strip reads "Couldn't read this page · try again" and can be pressed again — rather than publish a verdict from the platforms alone while claiming "0 on this page" (the owner chose this over a labelled platforms-only verdict). A shop's placement parameters no longer split one product across several pages. |
